...
class Anchor {
  ...
  get col() {
    const colWidth = Math.min(this.colWidth - 1, this.nativeColOff) / this.colWidth;
    return this.nativeCol + colWidth;
  }
  ...
  get row() {
    const rowHeight = Math.min(this.rowHeight - 1, this.nativeRowOff) / this.rowHeight;
    return this.nativeRow + rowHeight;
  }
  ...
  get colWidth() {
    ...
    const customColWidth =
      this.worksheet && this.worksheet.getColumn(this.nativeCol + 1).width
