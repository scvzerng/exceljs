...
const CellImagesXform = require('./xform/book/workbook-cellimages-xform');
...
class XLSX {
  ...
  parseCellImages(stream) {
    const cellImagesXform = new CellImagesXform();
    return cellImagesXform.parseStream(stream);
  }
    ...
    for (const entry of Object.values(zip.files)) {
      ...
      if (!entry.dir) {
        ...
        switch (entryName) {
          ...
          case 'xl/cellimages.xml': {
            model.cellImages = await this.parseCellImages(stream);
            break;
          }
          ...
          case 'xl/_rels/cellimages.xml.rels':
            model.cellImagesRels = await this.parseRels(stream);
            break;
