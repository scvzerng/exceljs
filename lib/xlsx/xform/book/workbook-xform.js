...
class WorkbookXform extends BaseXform {
  ...
  reconcile(model) {
    ...
    const imageMap = model.media
      .filter(item => item.type === 'image')
      .reduce((map, image) => {
        map.set(`media/${image.name}.${image.extension}`, image.buffer);
        return map;
      }, new Map());
    const imageRidMap = model.cellImagesRels.reduce((map, rel) => {
      map.set(rel.Id, rel.Target);
      return map;
    }, new Map());
    model.cellImages = model.cellImages.reduce((map, image) => {
      map.set(image.name, imageMap.get(imageRidMap.get(image.rId)));
      return map;
    }, new Map());
